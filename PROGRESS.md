...
1. 下一条具体操作：以 station_processing.py 的二维观测为基础，实现米制定位输入契约与求解；绑定真实标记尺寸、相机外参、场景变换和采集位姿，计算明确的定位不确定性，不把像素误差当 residual_m。不要重建仓库。
2. 已实现显式单次采集二维派生和证据详情；批量调度、米制定位、列表分页、静态 openapi.yaml 同步仍需实现。三维设备选择与历史读数面板尚未做交互验收。
