...
任务书范围的软件流程已形成未测试的开发候选，具体逐项见 docs/development/DELIVERY_MATRIX.md。OpenAPI、分页、人工复核、独立身份/角色、审计和部署隔离均已补齐。后续不要重建仓库或重新恢复基线。

- 原有“不运行测试”要求仍有效：全部单元、集成、E2E、性能、故障注入、权限与实车测试未执行。
- 现场尺寸、标定、PTP、路线/长期采集、误差预算及 Jetson ARM64 实机部署依赖现场输入。
- 在已提供的挑战杯交接范围内未找到客户数据库备份；真实单位/关系/阈值没有声称完成映射。通用无标记模型缺训练素材。
- M6 已准备真实融合前端和两阶段提示；根据此前要求未调用外部模型或多 agent。这不是已经完成 Kimi/Gemini 视觉优化。
- 下一次从实际验证结果或新提供输入继续修复/适配，不重复规划软件骨架。
...
- SQLite 固化批次清单/版本和逐项状态，追加事件历史；独立工作者锁保证后台单工作者，保留每项原归档锁与幂等报告。
- 暂停在当前项后生效；重启重排中断项，暂停状态保留；失败项可显式重试。版本不匹配暂停旧批次；报告和观测不覆盖。
- 队列表纳入备份账本摘要。备份前必须暂停并等待当前项结束，避免活跃状态变动造成摘要不一致。
- 新增游标分页和单记录读取；站点每页 25、报告每页 10、批次明细每页 25。旧数组 API 保留兼容，旧业务页面迁移留待下一步。
- ProcessingJobsPanel 连接创建、状态更新、暂停/恢复与失败项重试。详情报告可显式刷新，分页说明明确实时视图与页面内时间排序。
- Python 和 TypeScript/Vite 编译完成；未创建或执行任何实际批次，未运行处理接口、单元/集成/E2E/性能/故障注入/实车测试。恢复逻辑未经中断演练。

- 本阶段本地 API/预览已更新，启动日志正常；没有提交批次或调用处理接口。Vite 大资源块提示仍在。


## 软件收尾与交付候选（本阶段）

- 资产、观测、站点业务页分页；人工候选计算、确认/拒绝、追加历史和 expected_review_id 并发检查。认证模式下复核标签绑定实际身份。
- FIELD_AUTH_FILE 独立身份：viewer/reviewer/operator；逐人令牌和会话可撤销；共享密钥和本机模式兼容。写请求与业务事件保留认证主体。未发行真实人员凭据。
- OpenAPI 从实际路由生成，文档模式不打开数据库/启动 worker。增加凭据发行脚本和部署文档。
- Docker 冻结依赖安装，排除运行归档、数据库和私有配置；提供凭据只读挂载配置。未构建或启动容器。
- README、用户手册、开发说明和 DELIVERY_MATRIX 已按当前实现重写，前端模型交接已更新。原始许可证不变。
- 整套 pnpm build（契约、三维、网页、API）完成；Vite 仍有大资源块提示。没有执行任何测试、实际图片处理、批次或权限探测。

- 本阶段本地 API/网页预览已更新；API 日志确认 Application startup complete。仅查看启动日志，未调用业务接口或执行交互测试。
