...
1. 下一条具体操作：同步 instruments 新接口的 OpenAPI 文档，并迁移 InstrumentEvidencePage 的旧数组列表到分页；随后接入定位候选的人工复核界面。不重建仓库。
2. 已实现单次与批量派生、条件米制估计、站点/报告/批次分页。静态 OpenAPI 同步、旧业务页分页与人工复核页面仍待实现；交互验收未执行。
